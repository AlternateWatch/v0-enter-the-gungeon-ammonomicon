...
export function GunsPage({ searchQuery }: GunsPageProps) {
  ...
  const refreshGuns = async () => { const supabase = createClient(); const { data } = await supabase.from("guns").select<"*", Gun>("*").order("name"); setLocalGuns(data || []); }
  const handleSave = async () => { if (!editingGun?.name) return; setIsSaving(true); const supabase = createClient(); try { let imageUrl = editingGun.image_url; if (imageFile) { const formData = new FormData(); formData.append("file", imageFile); imageUrl = await uploadImage(formData); } const gunData = { ...editingGun, image_url: imageUrl, }; if (editingGun.id) { await supabase.from("guns").update(gunData).eq("id", editingGun.id).throwOnError(); } else { await supabase.from("guns").insert([gunData]).throwOnError(); } await refreshGuns(); setIsEditOpen(false); } catch (err) { setError(err instanceof Error ? err.message : "Error saving gun"); } finally { setIsSaving(false); } }
  const handleDelete = async (id: string) => { if (confirm("¿Estás seguro?")) { const supabase = createClient(); try { await supabase.from("guns").delete().eq("id", id).throwOnError(); setLocalGuns(prev => prev.filter(g => g.id !== id)); } catch (err) { setError(err instanceof Error ? err.message : "Error"); } } }
  ...
  return (
    <div className="p-6">
      ...
      {filteredGuns.length === 0 && !error && (<div className="text-center py-12"><p className="text-muted-foreground">No hay armas que coincidan con tu búsqueda.</p></div>)}
      {error && (<div className="text-center py-12"><p className="text-destructive">Error cargando armas: {error}</p></div>)}
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}><DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>{editingGun?.id ? "Editar Arma" : "Añadir Nueva Arma"}</DialogTitle></DialogHeader><div className="space-y-4 py-4">
        ...
        <div className="grid grid-cols-3 gap-4"><div><Label htmlFor="magazine_size">Cargador</Label><Input id="magazine_size" type="number" value={editingGun?.magazine_size || ""} onChange={(e) => setEditingGun({ ...editingGun, magazine_size: Number(e.target.value) })} /></div><div><Label htmlFor="max_ammo">Munición Máx.</Label><Input id="max_ammo" type="number" value={editingGun?.max_ammo || ""} onChange={(e) => setEditingGun({ ...editingGun, max_ammo: Number(e.target.value) })} /></div><div><Label htmlFor="reload_time">Recarga (s)</Label><Input id="reload_time" type="number" step="0.1" value={editingGun?.reload_time || ""} onChange={(e) => setEditingGun({ ...editingGun, reload_time: Number(e.target.value) })} /></div><div><Label htmlFor="damage">Daño</Label><Input id="damage" value={editingGun?.damage || ""} onChange={(e) => setEditingGun({ ...editingGun, damage: e.target.value })} /></div><div><Label htmlFor="dps">DPS</Label><Input id="dps" value={editingGun?.dps || ""} onChange={(e) => setEditingGun({ ...editingGun, dps: e.target.value })} /></div><div><Label htmlFor="fire_rate">Cadencia</Label><Input id="fire_rate" value={editingGun?.fire_rate || ""} onChange={(e) => setEditingGun({ ...editingGun, fire_rate: e.target.value })} /></div></div>
