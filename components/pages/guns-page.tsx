...
const isGunDataIncomplete = (gun: Gun): boolean => {
  ...
  for (const field of requiredFields) {
    if (gun[field] === null || gun[field] === '') return true;
  }
  return false;
};
...
export function GunsPage({ searchQuery }: GunsPageProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const gunData = { ...editingGun, image_url: imageUrl, magazine_size: editingGun.magazine_size || null, max_ammo: editingGun.max_ammo || null, reload_time: editingGun.reload_time || null, };
      if (editingGun.id) { await supabase.from("guns").update(gunData).eq("id", editingGun.id).throwOnError(); }
      else { await supabase.from("guns").insert([gunData]).throwOnError(); }
      await forceRefetch();
      setIsEditOpen(false);
    } catch (err) { setError(err instanceof Error ? err.message : "Error al guardar el arma."); }
