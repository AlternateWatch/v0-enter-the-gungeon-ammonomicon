...
export function BossesPage({ searchQuery }: BossesPageProps) {
  ...
  const handleSave = async () => { 
    ...
    try { 
      ...
      const bossData: Partial<Boss> = { ...editingBoss, image_url: imageUrl, is_duo: isDuo, phases: editingBoss.phases ? Number(editingBoss.phases) : null, health: editingBoss.health || null, health_2: editingBoss.health_2 || null, common_health: editingBoss.common_health || null, }; 
      ...
      if (editingBoss.id) { await supabase.from("bosses").update(bossData).eq("id", editingBoss.id).throwOnError(); } else { await supabase.from("bosses").insert([bossData]).throwOnError(); } 
      await forceRefetch(); 
      setIsEditOpen(false); 
    } catch (err) { setError(err instanceof Error ? err.message : "Error: Revisa que los campos numéricos (como Fases) contengan un número válido."); } 
  ...
  const handleDelete = async (id: string) => { 
    if (confirm("¿Estás seguro?")) { 
      const supabase = createClient(); 
      try { await supabase.from("bosses").delete().eq("id", id).throwOnError(); await forceRefetch(); } 
      catch (err) { setError(err instanceof Error ? err.message : "Error"); } 
    } 
  }

  const filteredBosses = localBosses.filter((boss) => { 
    ...
    <div className="p-6">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Guía completa de todos los jefes.</p>
        </div>
        <Button onClick={handleAddNew} className="gap-2"><Plus className="w-4 h-4" />Añadir Jefe</Button>
      </div>
      <div className="mb-6 flex flex-wrap gap-2">
        {floors.map((floor) => (
          <Button key={floor} variant={selectedFloor === floor ? "default" : "secondary"} onClick={() => setSelectedFloor(floor)}>
            {floor}
          </Button>
        ))}
      </div>
          ...
          return (
            <Card key={boss.id} onClick={() => handleCardClick(boss)} className="cursor-pointer hover:shadow-lg transition-shadow group relative flex flex-col">
              ...
              <div className="absolute top-2 right-2 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button size="icon" variant="secondary" className="h-8 w-8" onClick={(e) => { e.stopPropagation(); handleEdit(boss); }}>
                  <Pencil className="h-4 w-4" />
                </Button>
                <Button size="icon" variant="destructive" className="h-8 w-8" onClick={(e) => { e.stopPropagation(); handleDelete(boss.id); }}>
              ...
              <CardHeader>
                {boss.image_url && (
                  <div className="w-full h-40 mb-4 bg-muted rounded-lg flex items-center justify-center p-4">
                    <img src={boss.image_url} alt={boss.name || boss.common_name || "Jefe"} className="max-h-full max-w-full object-contain" style={{ imageRendering: "pixelated" }} />
                  </div>
                )}
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingBoss?.id ? "Editar Jefe" : "Añadir Jefe"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="flex items-center space-x-2">
              <Checkbox id="isDuo" checked={isDuo} onCheckedChange={(c) => setIsDuo(Boolean(c))} />
              <Label htmlFor="isDuo" className="font-medium">Es un dúo</Label>
            </div>
            ...
            {isDuo ? (
              <>
                {/* Datos Comunes y Entidades */}
                <h4 className="font-semibold text-lg pt-2">Datos Comunes</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div><Label htmlFor="common_name">Nombre Dúo *</Label><Input id="common_name" value={editingBoss?.common_name || ""} onChange={(e) => setEditingBoss({ ...editingBoss, common_name: e.target.value })} required /></div>
                  <div><Label>Piso</Label><Input value={editingBoss?.location || ""} onChange={(e) => setEditingBoss({ ...editingBoss, location: e.target.value })} /></div>
                  <div><Label>Vida Total</Label><Input value={editingBoss?.common_health || ""} onChange={(e) => setEditingBoss({ ...editingBoss, common_health: e.target.value })} /></div>
                </div>

                <Separator className="my-6" /><h4 className="font-semibold text-lg">Entidad 1</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div><Label>Nombre 1</Label><Input value={editingBoss?.name || ""} onChange={(e) => setEditingBoss({ ...editingBoss, name: e.target.value })} /></div>
                  <div><Label>Vida 1</Label><Input value={editingBoss?.health || ""} onChange={(e) => setEditingBoss({ ...editingBoss, health: e.target.value })} /></div>
                </div>
                  ...
                  <div className="mt-2 space-y-4">
                    {imagePreview ? (
                      <div className="relative">
                        <div className="w-full h-64 bg-muted rounded-lg flex items-center justify-center p-4">
                          <img src={imagePreview} alt="Preview" className="max-h-full max-w-full object-contain" style={{ imageRendering: "pixelated" }} />
                      ...
                      <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                        ...
                        <p className="text-sm text-muted-foreground mb-2">Sube una imagen</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><Label>Nombre 2</Label><Input value={editingBoss?.name_2 || ""} onChange={(e) => setEditingBoss({ ...editingBoss, name_2: e.target.value })} /></div>
                  <div><Label>Vida 2</Label><Input value={editingBoss?.health_2 || ""} onChange={(e) => setEditingBoss({ ...editingBoss, health_2: e.target.value })} /></div>
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 space-y-4">
                    {imagePreview2 ? (
                      <div className="relative">
                        <div className="w-full h-64 bg-muted rounded-lg flex items-center justify-center p-4">
                          <img src={imagePreview2} alt="Preview 2" className="max-h-full max-w-full object-contain" style={{ imageRendering: "pixelated" }} />
                          <Button type="button" variant="destructive" size="icon" className="absolute top-2 right-2" onClick={handleRemoveImage2}>
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                        <Upload className="h-8 w-8 mx-auto mb-2 text-muted-foreground" />
                        <p className="text-sm text-muted-foreground mb-2">Sube una imagen</p>
                        <Input type="file" accept="image/*" onChange={handleImageChange2} className="max-w-xs mx-auto" />
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div><Label>Fases</Label><Input type="number" value={editingBoss?.phases || ""} onChange={(e) => setEditingBoss({ ...editingBoss, phases: e.target.value })} /></div>
              ...
              <>
                {/* Caso de un solo jefe */}
                <div className="grid grid-cols-2 gap-4">
                  <div><Label>Nombre *</Label><Input value={editingBoss?.name || ""} onChange={(e) => setEditingBoss({ ...editingBoss, name: e.target.value })} required /></div>
                  <div><Label>Piso</Label><Input value={editingBoss?.location || ""} onChange={(e) => setEditingBoss({ ...editingBoss, location: e.target.value })} /></div>
                  <div><Label>Vida</Label><Input value={editingBoss?.health || ""} onChange={(e) => setEditingBoss({ ...editingBoss, health: e.target.value })} /></div>
                </div>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button onClick={handleSave} disabled={isSaving || (!isDuo && !editingBoss?.name) || (isDuo && !editingBoss?.common_name)}>
