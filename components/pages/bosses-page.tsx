...
interface BossesPageProps { searchQuery: string }

const floors = ["Todos los pisos", "Fortaleza del Señor del Plomo", "Armazmorra en Sí", "Mina de Pólvora", "Hondanada", "Forja", "Infierno de las Balas", "Calabozo", "Abadía del Arma Verdadera", "Guarida de la Rata", "Departamento de I+D", "Pasado"];
const isBossDataIncomplete = (boss: Boss): boolean => { const base: (keyof Boss)[] = ['attacks', 'description', 'health', 'image_url', 'location', 'phases', 'strategy', 'quote']; if (boss.is_duo) { const duo: (keyof Boss)[] = [...base, 'common_name', 'name', 'name_2', 'description_2', 'health_2', 'image_url_2', 'quote_2']; for (const f of duo) if (!boss[f]) return true; } else { const single: (keyof Boss)[] = [...base, 'name']; for (const f of single) if (!boss[f]) return true; } return false; };
...
export function BossesPage({ searchQuery }: BossesPageProps) {
  ...
  const refreshBosses = async () => { const supabase = createClient(); const { data } = await supabase.from("bosses").select<"*", Boss>("*").order("name"); setLocalBosses(data || []); }
  const handleSave = async () => { if (!editingBoss) return; if ((!isDuo && !editingBoss.name) || (isDuo && !editingBoss.common_name)) return; setIsSaving(true); const supabase = createClient(); try { let imageUrl = editingBoss.image_url, imageUrl2 = editingBoss.image_url_2; if (imageFile) { const fd = new FormData(); fd.append("file", imageFile); imageUrl = await uploadImage(fd); } if (imageFile2) { const fd = new FormData(); fd.append("file", imageFile2); imageUrl2 = await uploadImage(fd); } const bossData: Partial<Boss> = { ...editingBoss, image_url: imageUrl, is_duo: isDuo }; if (isDuo) { bossData.image_url_2 = imageUrl2; } else { Object.assign(bossData, { name_2: null, image_url_2: null, description_2: null, quote_2: null, health_2: null, common_name: null, common_health: null }); } if (editingBoss.id) { await supabase.from("bosses").update(bossData).eq("id", editingBoss.id).throwOnError(); } else { await supabase.from("bosses").insert([bossData]).throwOnError(); } await refreshBosses(); setIsEditOpen(false); } catch (err) { setError(err instanceof Error ? err.message : "Error"); } finally { setIsSaving(false); } }
  const handleDelete = async (id: string) => { if (confirm("¿Estás seguro?")) { const supabase = createClient(); try { await supabase.from("bosses").delete().eq("id", id).throwOnError(); setLocalBosses(prev => prev.filter(b => b.id !== id)); } catch (err) { setError(err instanceof Error ? err.message : "Error"); } } }
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-start justify-between"><div><h2 className="text-3xl font-bold text-foreground mb-2">Jefes</h2><p className="text-muted-foreground">Guía completa de todos los jefes.</p></div><Button onClick={handleAddNew} className="gap-2"><Plus className="h-4 w-4" />Añadir Jefe</Button></div>
          ...
          <div className="space-y-4 py-4">
            ...
            {isDuo ? (
              <>
                ...
                <h4 className="font-semibold text-lg">Entidad 1</h4>
                ...
                <Separator className="my-6" />
                <h4 className="font-semibold text-lg">Entidad 2</h4>
                ...
                <Separator className="my-6" />
                <h4 className="font-semibold text-lg">Notas Comunes</h4>
